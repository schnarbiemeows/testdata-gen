...
import com.schnarbiesnmeowers.testdatagen.generators.{BooleanTypeGenerator, DateTimeTypeGenerator, DateTypeGenerator, DecimalNumberTypeGenerator, StringTypeGenerator, WholeNumberTypeGenerator}
...
object MakeGenericRecord {
  ...
  def makeRecord(records: RecordsTemplate, arrayNum: Int, fileNum: Int):Record = {
    ...
    val fieldValues = new Array[String](fields.length)
    ...
    for(i <- 0 until fields.length) {
      ...
      dataType match {
        ...
        case "RandomDate" => fieldValues(i) = DateTypeGenerator.makeRandomDate("RandomDate",format,qualifiers)
        case "RangedDate" => fieldValues(i) = DateTypeGenerator.makeRangedDate("RangedDate",format,qualifiers,arrayNum,fileNum)
        ...
        case "RangedDateTime" => fieldValues(i) = DateTimeTypeGenerator.makeRangedDateTime("RangedDateTime",format,qualifiers,arrayNum,fileNum)
